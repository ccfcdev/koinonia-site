...
function register(){ const f = $('.reg'); if (!f) return; const g = $('.reg__gform'), status = $('.reg__status', f), map = window.KOI_GFORM || {};
  f.addEventListener('submit', async e => { e.preventDefault(); let ok = true;
    ...
    try { if (g){ Object.entries(map).forEach(([k, name]) => { const inp = g.querySelector(`[name="${name}"]`); if (inp) inp.value = d[k] ?? ''; }); g.submit(); }
      const row = { site:'koinonia', edition: f.dataset.edition, first_name:d.first, surname:d.surname, gender:d.gender, age_range:d.age, residence:d.residence, phone:d.phone, email:d.email, participation:d.participation, participation_detail:d.detail, days:d.days, dietary:d.dietary, expectation:d.expectation };
      const cfg = window.CCFC_CONFIG || {};
      if (cfg.supabaseUrl && cfg.supabaseKey){ const r = await fetch(cfg.supabaseUrl + '/rest/v1/registrations', { method:'POST', headers:{ 'Content-Type':'application/json', apikey: cfg.supabaseKey, Authorization: 'Bearer ' + cfg.supabaseKey, Prefer: 'return=minimal' }, body: JSON.stringify(row) }); if (!r.ok) throw new Error('registration ' + r.status); }
...
function boot(){ register(); split(); grain(); nav(); reveals(); hero(); lightbox(); countdown(); capture(); $$('.year').forEach(e => e.textContent = new Date().getFullYear()); }
